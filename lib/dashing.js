...
function loadRessourceJobs(pathToJobs, logger) {
    return when.promise(function (resolve, reject) {
        ...
        logger.info("Loading jobs from", pathToJobs, "...");

        fs.readdir(pathToJobs, function (err, files) {
            if (err) {
                logger.error("loading jobs", err.message);
                reject(err);
            }
            ...
            jobs.forEach(function (filename) {
                ...
                if (file.match(/(\w*)\.job\.(js|coffee)$/)) {
                    logger.info("loading job file:", filename);

                    try {
                        require(file);
                        logger.info("job loaded", file);

                    } catch (e) {
                        logger.error("failed loading job", e.message, e.stack);
                    }
                }
            });
